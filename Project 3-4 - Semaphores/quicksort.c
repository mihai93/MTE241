...
__task void quick_sort_task( void* void_ptr){
	...
	left_param.depth = param->depth + 1;
	right_param.depth = param->depth + 1;
	
	os_sem_wait(&global.sem,0xffff);
	global.depth = param->depth + 1;
	os_sem_send(&global.sem);
	...
	os_tsk_create_ex( quick_sort_task, left_param.depth, &left_param ); 
	
	while(1)
	{
		os_sem_wait(&global.sem,0xffff);
		if(global.depth == param->depth)
			break;
		os_sem_send(&global.sem);
	}
	
	os_tsk_create_ex( quick_sort_task, right_param.depth, &right_param );
...
void quicksort( array_t array ) {
	...
	global.depth = 1;
	os_sem_init(&global.sem, 1);

	// If you are using priorities, you can change this
	task_param.depth = 1;
	
	//start the quick_sort threading
	os_tsk_create_ex( quick_sort_task, task_param.depth, &task_param ); 
